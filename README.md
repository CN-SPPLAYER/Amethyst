...
<img src="https://img.shields.io/badge/version-1.0.5-9b59b6?style=for-the-badge" alt="version">
...
> 主界面右侧面板的“从右向左”滑入动画为内置行为，**强制开启**，不受“界面优化”总开关控制。

菜单中的 **界面优化** 一个总开关，一键开启全部界面/显示功能：
...
- 落雪效果、装扮保存（仓库页 6 个预设按钮，一键换装）
- 解锁 240 FPS、解锁全部装扮、跳过断线惩罚、自动返回大厅 在“显示与身份”中单独开关（**不随“界面优化”总开关联动**）
...
5. 将 **`Amethyst_v1.0.5.dll`** 放入 `BepInEx/plugins/`
...
构建产物为 `src/bin/Release/net6.0/Amethyst.dll`（及带版本号副本 `Amethyst_v1.0.5.dll`）。
